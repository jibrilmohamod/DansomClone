<template>
 <div class="text-white">
  ...
  <section class="container mx-auto px-6 py-16 space-y-12">
   <div class="rounded-3xl border border-white/10 bg-white/5 p-8 shadow-glow backdrop-blur">
    <div class="grid items-center gap-8 md:grid-cols-2">
     ...
     <div class="grid gap-4 sm:grid-cols-2">
      <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
       <p class="text-lg font-semibold text-white">Secure access</p>
       <p class="text-sm text-white/70">Field teams positioned across Somalia and Kenya.</p>
      </div>
      <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
       <p class="text-lg font-semibold text-white">Decision-ready</p>
       <p class="text-sm text-white/70">Clear deliverables aligned to partner objectives.</p>
      </div>
     </div>
    </div>
   </div>
   ...
   <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
    <div
     ...
     :key="service.title"
     class="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-glow backdrop-blur transition hover:-translate-y-2 hover:border-secondary/40"
    >
     <div class="flex items-center gap-3">
      ...
      <p class="text-xs uppercase tracking-[0.2em] text-white/70">{{ service.category }}</p>
     </div>
     <h3 class="mt-3 text-xl font-semibold">{{ service.title }}</h3>
     <p class="mt-2 text-sm text-white/70 leading-relaxed">{{ service.description }}</p>
     <NuxtLink
      :to="`/Services/${encodeURIComponent(service.title)}`"
      class="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-secondary transition hover:translate-x-1"
     >
      Explore
