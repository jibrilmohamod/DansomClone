<template>
 <div class="relative min-h-screen overflow-hidden text-slate-900">
  <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_14%_22%,rgba(80,174,211,0.16),transparent_30%),radial-gradient(circle_at_84%_12%,rgba(191,217,98,0.14),transparent_30%)]"></div>
  <div class="relative">
  ...
  <div class="container m-auto px-6 py-14">
   <div class="dansom-card grid gap-8 p-8 md:grid-cols-3">
    <div class="space-y-4">
     <div class="flex items-center justify-center rounded-2xl bg-gradient-to-br from-secondary/20 via-white to-quinary/25 p-6 ring-1 ring-secondary/20">
      <Icon :name="service.icon" class="text-primary h-20 w-20 animate-pulse" />
     </div>
     <div class="dansom-card-soft p-4">
      <p class="text-xs uppercase tracking-[0.2em] text-slate-500">Delivery model</p>
      <p class="mt-2 text-sm font-semibold text-primary">Mixed-method research + local field teams</p>
     </div>
     <div class="dansom-card-soft p-4">
      <p class="text-xs uppercase tracking-[0.2em] text-slate-500">Coverage</p>
      <p class="mt-2 text-sm font-semibold text-primary">Somalia and Kenya operations</p>
     </div>
    </div>
    <div class="md:col-span-2 space-y-4">
     <h5 class="font-DM text-3xl md:text-4xl font-bold text-primary">{{ service.fullTitle }}</h5>
     <p class="font-Zilla text-slate-700 text-lg leading-relaxed">{{ service.subtitle }}</p>
     <div class="prose prose-slate max-w-none prose-headings:text-primary prose-a:text-secondary" v-html="service.about"></div>
    </div>
   </div>
  </div>
  </div>
